...
class rc4_t
{
    ...
    string codificar_mensaje(string mensaje,vector<int> semilla)
    {
        ...
        for(int i = 0; i < codigo_ascii.size();i++)
        {
            aux_mensaje = to_binary(codigo_ascii[i]);
    ...
    vector<int> descifrar_mensaje(string mensaje)
    {
        ...
        {
            aux_mensaje = to_binary(codigo_ascii[i]);
            aux_semilla = to_binary(Cifrado_[k]);
            k++;
            // cout <<"mensaje"<< mensaje[i] << " __ "<<aux_mensaje << endl;
            // cout <<"semilla"<< Cifrado_[i] << "__ " <<aux_semilla << endl;
            
            
            resultado = xor_(aux_mensaje,aux_semilla);
            ...
            for(int j = resultado.size()-1; j >= 0 ; j--)
            {
                ...
                acumulado+=(pow(2,contador_ocho)*aux);
                contador_ocho++;
            }
            //cout << "valor de i "<< i<<"_" <<acumulado << endl;
            char resultado_char = acumulado;
            resultado1.push_back(resultado_char);
            acumulado = 0;
        }
